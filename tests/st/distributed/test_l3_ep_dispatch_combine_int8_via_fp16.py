...
def _build_ep_dispatch_combine_program():
    ...
    @pl.program
    class EpDispatchCombineInt8ViaFp16:
        # ----------------------------------------------------------------
        ...
        def dispatch_step(  # noqa: PLR0913, PLR0912, PLR0915
            ...
            recv_x: pld.DistributedTensor[[L * R, D], pl.FP16],
            ...
            for t in pl.range(T):
                for k in pl.range(TOPK):
                    ...
                    # Channel 1: x INT8 → cast to FP16 → push as FP16
                    # (experiment 1: bypass b8 cross-rank intrinsic).
                    x_tile = pl.load(x_norm, [t, 0], [1, D])
                    x_tile_f16 = pl.tile.cast(x_tile, target_type=pl.FP16)
                    pld.tile.remote_store(x_tile_f16, target=recv_x, peer=dst, offsets=[row, 0])
            ...
            for e in pl.range(L):
                for slot in pl.range(R):
                    row = e * R + slot
                    x_tile_f16 = pl.load(recv_x, [row, 0], [1, D])
                    x_tile = pl.tile.cast(x_tile_f16, target_type=pl.INT8)
        ...
        def host_orch(
            ...
            recv_x_buf = pld.alloc_window_buffer(L * R * D * 2)  # FP16 (was INT8)
            ...
            for r in pl.range(pld.world_size()):
                ...
                recv_x = pld.window(recv_x_buf, [L * R, D], dtype=pl.FP16)
    ...
    return EpDispatchCombineInt8ViaFp16
...
class TestL3EpDispatchCombineInt8ViaFp16Int8:
    """L3 distributed runtime: 2-rank EP dispatch + local_expert + combine, INT8 x."""

    def test_ep_dispatch_combine_int8_via_fp16(self, test_config, device_ids):
        if len(device_ids) < 2:
