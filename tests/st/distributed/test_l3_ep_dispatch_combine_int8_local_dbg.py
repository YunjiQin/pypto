...
"""L3 distributed st: INT8-x variant of ``test_l3_ep_dispatch_combine``.

Identical to the sibling test in every dimension, channel layout, barrier,
and golden — the **only** delta is that the ``x`` channel is INT8 instead of
BF16 (window + host-backed staged output + golden compute path).

This is a focused regression for an a2a3 hardware bug in the cross-rank INT8
TLOAD/TSTORE path: with x = BF16 the test passes; flipping just the x dtype
to INT8 (no other change) makes every ``recv_x`` row mismatch with errors
covering the full INT8 range, while ``recv_count`` / ``recv_w`` / ``recv_idx``
all match. So the bug is isolated to ``pld.tile.remote_store`` /
``pld.tile.remote_load`` on INT8 — most likely the ``copy_ubuf_to_gm_align_b8``
burst path or the b8 cacheline coherence — and not in the protocol,
notify/wait barriers, prefix_sum, slot-offset arithmetic, or stage_out
machinery, all of which are exercised identically here.

The ``local_expert`` kernel's matmul uses ``cast_fp32(int8) * fp32_w``, so
``recv_y`` and ``routed_y_buf`` stay BF16 (same end-of-pipeline dtype as the
BF16-x variant). The routed-y golden therefore reuses the BF16 round-trip.
"""
...
def _build_ep_dispatch_combine_program():
    ...
    @pl.program
    class EpDispatchCombineInt8LocalDbg:
        # ----------------------------------------------------------------
        ...
        def dispatch_step(  # noqa: PLR0913, PLR0912, PLR0915
            ...
            for t in pl.range(T):
                for k in pl.range(TOPK):
                    ...
                    # Channel 1: x INT8 [1, D]
                    # Experiment 2: TSTORE the same INT8 tile to a LOCAL host-backed
                    # buffer right before the cross-rank push. If b8 TSTORE works on
                    # local GM but not on cross-rank GM, local_x_dbg will contain the
                    # data while recv_x_out remains zero.
                    x_tile = pl.load(x_norm, [t, 0], [1, D])
                    pl.store(x_tile, [r_route, 0], local_x_dbg)
            ...
            return recv_x_out, recv_w_out, recv_idx_out, recv_count_out, local_x_dbg
        ...
        def local_expert_step(
            self,
            recv_x_out: pl.Tensor[[L * R, D], pl.INT8],
        ...
        def chip_orch(  # noqa: PLR0913
            ...
            recv_x_out, recv_w_out, recv_idx_out, recv_count_out, local_x_dbg = self.dispatch_step(
                indices,
                x_norm,
                w_padded,
                idx_padded,
        ...
        def host_orch(
            ...
            local_x_dbgs: pl.Out[pl.Tensor[[N_RANKS, N_ROUTES, D], pl.INT8]],
            ...
            recv_x_buf = pld.alloc_window_buffer(L * R * D * 1)  # INT8
            ...
            for r in pl.range(pld.world_size()):
                ...
                recv_x = pld.window(recv_x_buf, [L * R, D], dtype=pl.INT8)
    ...
    return EpDispatchCombineInt8LocalDbg
...
def _compute_golden_recv(
    ...
    """
    expected_recv_x = torch.zeros(N_RANKS, L, R, D, dtype=torch.int8)
...
class TestL3EpDispatchCombineInt8LocalDbgInt8:
    """L3 distributed runtime: 2-rank EP dispatch + local_expert + combine, INT8 x."""

    def test_ep_dispatch_combine_int8_local_dbg(self, test_config, device_ids):
        if len(device_ids) < 2:
            pytest.skip(f"ep_dispatch_combine_int8 needs 2 devices, got {device_ids}")
        ...
        # Structured INT8 inputs clamped to [-125, 125]. Same per-(r, t, d)
        # locality pattern as the BF16-x sibling test so any mismatch can be
        # traced back to a specific (r, t, d) without ambiguity.
        x_norms = torch.tensor(
            [
        ...
        recv_x_outs = torch.zeros((N_RANKS, L * R, D), dtype=torch.int8)
        ...
        local_x_dbgs = torch.zeros((N_RANKS, N_ROUTES, D), dtype=torch.int8)
        ...
        # ---------- experiment 2 — local TSTORE baseline ----------
        # local_x_dbg[r, r_route, :] receives a LOCAL b8 TSTORE of the same
        # INT8 tile that gets cross-rank-pushed into recv_x. If b8 TSTORE
        # works on local GM, local_x_dbg matches expected even when
        # recv_x_out (from cross-rank push) is all zeros.
        expected_local_x_dbg = torch.zeros((N_RANKS, N_ROUTES, D), dtype=torch.int8)
        for r in range(N_RANKS):
            for t in range(T):
                for k in range(TOPK):
                    expected_local_x_dbg[r, t * TOPK + k, :] = x_norms[r, t, :]
        # Print status before any assertion so both signals are visible.
        local_match = torch.equal(local_x_dbgs, expected_local_x_dbg)
        local_nonzero = bool((local_x_dbgs != 0).any().item())
        print(
            f"[EXP2] local_x_dbg match={local_match} nonzero={local_nonzero} "
            f"first_row_got={local_x_dbgs[0, 0, :8].tolist()} "
            f"first_row_exp={expected_local_x_dbg[0, 0, :8].tolist()}"
        )
        ...
        assert local_match, (
            f"local_x_dbg mismatch — local b8 TSTORE itself is broken. "
            f"got[0,0,:16]={local_x_dbgs[0, 0, :16].tolist()} "
            f"exp[0,0,:16]={expected_local_x_dbg[0, 0, :16].tolist()}"
        )
        ...
        for r in range(N_RANKS):
            for e in range(L):
                ...
                if n == 0:
                    continue
                got_x = recv_x_outs_4d[r, e, :n, :]
                exp_x = expected_recv_x[r, e, :n, :]
                assert torch.equal(got_x, exp_x), (
                    f"recv_x mismatch at rank {r} expert {e}: max abs diff = "
                    f"{(got_x.to(torch.int32) - exp_x.to(torch.int32)).abs().max().item()}"
                )
