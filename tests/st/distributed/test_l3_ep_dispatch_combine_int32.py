...
"""L3 distributed st: INT32-x variant of ``test_l3_ep_dispatch_combine``.

Identical to the sibling test in every dimension, channel layout, barrier,
and golden — the **only** delta is that the ``x`` channel is INT32 instead
of BF16 (window + host-backed staged output + golden compute path).

This complements the ``INT8`` regression: with x = INT8 the test fails
hard (recv_x rows return all-zero / pre-write data on a2a3), but with
x = INT32 the same protocol passes. The two together pin the failure
to the ``b8`` (single-byte) burst path of the cross-rank
``pld.tile.remote_store`` / ``remote_load`` lowering —
``copy_ubuf_to_gm_align_b8`` and its peer-side counterpart — rather
than to anything dtype-agnostic. The ``b16`` (BF16) and ``b32`` (INT32 /
FP32) paths through the same DSL ops are unaffected.

``local_expert`` reads ``recv_x_out`` as INT32 and uses
``cast_fp32(int32) * fp32_w`` (the cast is a no-op precision-wise for
small integer values); ``recv_y`` and ``routed_y_buf`` stay BF16 so
the rest of the pipeline is identical to the BF16-x sibling.
"""
...
def _build_ep_dispatch_combine_program():
    ...
    class EpDispatchCombineInt32:
        ...
        def dispatch_step(  # noqa: PLR0913, PLR0912, PLR0915
            ...
            recv_x: pld.DistributedTensor[[L * R, D], pl.INT32],
        ...
        def local_expert_step(
            self,
            recv_x_out: pl.Tensor[[L * R, D], pl.INT32],
        ...
        def host_orch(
            ...
            recv_x_buf = pld.alloc_window_buffer(L * R * D * 4)  # INT32
            ...
            for r in pl.range(pld.world_size()):
                ...
                recv_x = pld.window(recv_x_buf, [L * R, D], dtype=pl.INT32)
...
def _compute_golden_recv(
    ...
    """
    expected_recv_x = torch.zeros(N_RANKS, L, R, D, dtype=torch.int32)
...
class TestL3EpDispatchCombineInt32:
    """L3 distributed runtime: 2-rank EP dispatch + local_expert + combine, INT8 x."""

    def test_ep_dispatch_combine_int32(self, test_config, device_ids):
        if len(device_ids) < 2:
            pytest.skip(f"ep_dispatch_combine_int32 needs 2 devices, got {device_ids}")
        ...
        # Structured INT32 inputs — same per-(r, t, d) locality pattern as
        # the BF16-x / INT8-x siblings, but the integer range is widened
        # to exercise the full 4-byte payload of each element.
        x_norms = torch.tensor(
            [[[r * 100000 + t * 1000 + d for d in range(D)] for t in range(T)] for r in range(N_RANKS)],
            dtype=torch.int32,
        )
        weights = torch.tensor(
            [
        ...
        recv_x_outs = torch.zeros((N_RANKS, L * R, D), dtype=torch.int32)
        ...
        # Relative tolerance — the wide INT32 input range (~10⁵) feeds the
        # local_expert ``cast_bf16(cast_fp32(x) * w)`` path with BF16 ULP
        # noise ~1e-3 at the resulting magnitude. The dispatch goldens above
        # remain bit-exact.
