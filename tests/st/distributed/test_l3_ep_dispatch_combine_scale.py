...
"""L3 distributed st: 4-channel + moe_ep-scale variant of
``test_l3_ep_dispatch_combine``.

Same dispatch / local_expert / combine protocol as the BF16-x sibling
but with three deliberate changes that collectively replicate the
moe_ep model's dispatch shape:

1. Dimensions bumped to ``T=128 / D=4096 / L=8 / R=128`` (moe_ep DEMO).
2. An extra ``scale`` FP32 channel pushed alongside ``x`` / ``w`` /
   ``idx`` — four cross-rank ``pld.tile.remote_store`` ops per (t, k)
   instead of three.
3. ``data_done`` barrier collapsed into ``count_done`` (cell goes
   0 → 1 after the count phase, 1 → 2 after the data phase) so the
   four extra tensor args fit under ``MAX_TENSOR_ARGS=16``.

All three changes mirror exactly what moe_ep does. This test passes on
a2a3 — confirming that bumped dimensions, the 4th 32 B channel, and
count_done reuse are all safe in isolation. The companion
``test_l3_ep_dispatch_combine_int8`` regression is then the only one
that fails on a2a3, isolating the bug to INT8-dtype cross-rank
``pld.tile.remote_store`` / ``remote_load`` (suspected
``copy_ubuf_to_gm_align_b8`` path).

The ``scale`` channel is dispatch-only; ``local_expert`` and ``combine``
stay as in the sibling test, so the ``routed_y`` golden still validates
the BF16 round-trip end-to-end (with a slightly looser tolerance for
BF16 ULP noise at ``D=4096``).
"""
...
# Dimensions bumped to match the moe_ep DEMO config (T=128, D=2048-ish,
# L=8, R=128). D is held at 2048 rather than moe_ep's 4096 because BF16
# [1, D] tiles cap at ~4 KB before local_expert / combine's VEC budget
# starts spilling — what we want to isolate here is whether the
# dispatch-stage 4-channel push survives a moe_ep-scale payload, not
# whether the downstream pipeline fits in VEC.
N_RANKS = 2
T = 128
TOPK = 2
D = 4096
...
def _build_ep_dispatch_combine_program():
    ...
    class EpDispatchCombine:
        ...
        def dispatch_step(  # noqa: PLR0913, PLR0912, PLR0915
            ...
            recv_scale_out: pl.Out[pl.Tensor[[L, R], pl.FP32]],
            ...
            # ``count_done`` is reused for the data_done barrier (cell goes
            # 0 → 1 after count phase, 1 → 2 after data phase) so we can fit
            # the extra ``scale`` channel inside MAX_TENSOR_ARGS=16. Same
            # consolidation moe_ep does — see the data_done block below.
            count_done: pld.DistributedTensor[[N_RANKS, 1], pl.INT32],
            recv_x: pld.DistributedTensor[[L * R, D], pl.BF16],
            recv_scale: pld.DistributedTensor[[L * R, W_PAD], pl.FP32],
            ...
            # recv_scale_out / recv_w_out: per-expert TLOAD [R, W_PAD]
            # + row_sum (compacts along W_PAD axis; sum recovers slot [0]
            # since [1, W_PAD) is zero by design) + reshape [R, 1] → [1, R]
            # + TSTORE. Mirrors the runtime's TROWSUM stage_out for FP32
            # channels.
            for e in pl.range(L):
                s_wide: pl.Tile[[R, W_PAD], pl.FP32] = pl.load(recv_scale, [e * R, 0], [R, W_PAD])
                s_tmp: pl.Tile[[R, 1], pl.FP32] = pl.tile.create(
                    [R, 1], dtype=pl.FP32, target_memory=pl.MemorySpace.Vec
                )
                s_sum: pl.Tile[[R, 1], pl.FP32] = pl.tile.row_sum(s_wide, s_tmp)
                s_row: pl.Tile[[1, R], pl.FP32] = pl.tile.reshape(s_sum, [1, R])
                pl.store(s_row, [e, 0], recv_scale_out)
            ...
            return recv_x_out, recv_scale_out, recv_w_out, recv_idx_out, recv_count_out
        ...
        def host_orch(
            ...
            scale_padded: pl.Tensor[[N_RANKS, T, W_PAD], pl.FP32],
            ...
            recv_scale_outs: pl.Out[pl.Tensor[[N_RANKS, L, R], pl.FP32]],
            ...
            # signals are sized [N_RANKS, 1] to host per-src cells; this
            # variant reuses count_done for both count + data phases (see
            # dispatch_step docstring), so no separate data_done window.
            ...
            recv_scale_buf = pld.alloc_window_buffer(L * R * W_PAD * 4)  # FP32
            ...
            for r in pl.range(pld.world_size()):
                ...
                recv_scale = pld.window(recv_scale_buf, [L * R, W_PAD], dtype=pl.FP32)
...
def _compute_golden_recv(
    ...
    Mirrors runtime ``compute_golden`` in main.py, with one extra channel:
      expected_recv_x[r]     BF16  [L, R, D]
      expected_recv_scale[r] FP32  [L, R]   (per-token scale, new)
    ...
    expected_recv_scale = torch.zeros(N_RANKS, L, R, dtype=torch.float32)
        ...
        for src in range(N_RANKS):
            ...
            for t in range(T):
                for k in range(TOPK):
                    ...
                    expected_recv_scale[dst, loc_e, slot] = scales[src, t]
    ...
    return (expected_recv_x, expected_recv_scale, expected_recv_w, expected_recv_idx, expected_count)
...
class TestL3EpDispatchCombineScale:
    """L3 distributed runtime: 4-channel EP dispatch (x + scale + w + idx)."""

    def test_ep_dispatch_combine_scale(self, test_config, device_ids):
        if len(device_ids) < 2:
            pytest.skip(f"ep_dispatch_combine_scale needs 2 devices, got {device_ids}")
        ...
        # Structured inputs clamped to [-128, 127] — every value in that
        # range is exact in BF16 (mantissa large enough), so the dispatch
        # round-trip golden can use ``torch.equal`` without false negatives
        # from BF16 rounding at large D.
        x_norms = torch.tensor(
            [
                [[((r * 13 + t * 7 + d) % 251) - 125 for d in range(D)] for t in range(T)]
                for r in range(N_RANKS)
            ],
            dtype=torch.bfloat16,
        )
        # Per-token scale values (one per (r, t)); padded to [T, W_PAD] for
        # the same TILE shape as the w channel.
        scales = torch.tensor(
            [[0.5 + 0.01 * t + 0.001 * r for t in range(T)] for r in range(N_RANKS)],
            dtype=torch.float32,
        )
        scale_padded = torch.zeros((N_RANKS, T, W_PAD), dtype=torch.float32)
        scale_padded[:, :, 0] = scales
        ...
        recv_scale_outs = torch.zeros((N_RANKS, L, R), dtype=torch.float32)
        ...
        (expected_recv_x, expected_recv_scale, expected_recv_w, expected_recv_idx, expected_count) = (
            _compute_golden_recv(x_norms, indices, weights, scales)
        )
        ...
        for r in range(N_RANKS):
            for e in range(L):
                ...
                got_s = recv_scale_outs[r, e, :n]
                exp_s = expected_recv_scale[r, e, :n]
                assert torch.allclose(got_s, exp_s, atol=1e-6), (
                    f"recv_scale mismatch at rank {r} expert {e}: max diff = {(got_s - exp_s).abs().max().item()}"
                )
        ...
        # Looser atol than the 3-channel sibling because at moe_ep-scale
        # dimensions (T=128, D=2048) the BF16 round-trip in local_expert's
        # ``cast_bf16(cast_fp32(x) * w)`` plus the FP32 reduce can drift by a
        # few BF16 ULPs at magnitude ~10³ — that's numerics, not a protocol
        # mismatch. The dispatch-stage goldens above stay strict.
        expected_routed = _compute_golden_routed(x_norms, weights)
        rel_diff = (routed_ys - expected_routed).abs() / expected_routed.abs().clamp_min(1e-3)
        max_rel = rel_diff.max().item()
        assert max_rel < 0.05, f"ep_dispatch_combine routed_y mismatch: max relative diff = {max_rel:.4f}"
